...
async function afficherMeilleurFilm() {
  try {
    ...
    if (!reponseMeilleursFilms.ok)
      throw new Error("Erreur réseau - liste meilleurs films");
...
async function afficherFilmsCategorie(p_urlPage1, p_categorie) {
  try {
    ...
    if (!reponseFilmsPage1.ok)
      throw new Error("Erreur réseau - liste films de la page 1");
    ...
    if (dataListeFilmsPage1.next) {
      ...
      if (!reponseFilmsPage2.ok)
        throw new Error("Erreur réseau - liste films de la page 2");
    ...
    for (let i = 0; i < listeFilms.length; i++) {
      ...
      if (!reponseUrlFilm.ok)
        throw new Error("Erreur réseau - détail des film");
      ...
      boutonDetails.setAttribute(
        "aria-label",
        `Voir les détails du film ${film.title}`,
      );
...
async function afficherGenres() {
  try {
    ...
    if (!reponseGenresPage1.ok)
      throw new Error("Erreur réseau - liste des genres de la page 1");
    ...
    if (!dataListeGenrePage1.next)
      throw new Error("Erreur réseau - Impossible de changer de page");

    while (urlPageSuivante) {
      const reponseGenrePageSuivante = await fetch(urlPageSuivante);
      if (!reponseGenrePageSuivante.ok)
        throw new Error("Erreur réseau - Impossible de changer de page");
    ...
    for (let index = 0; index < listeGenre.length; index++) {
      ...
      nouvelleOption.addEventListener("click", (event) => {
        ...
        const listeLi = document
          .getElementById("select-genres")
          .querySelectorAll("li");
        listeLi.forEach((li) => {
        ...
        const urlPage1 =
          urlBaseTitre + `?genre=${genreChoisi}&sort_by=-imdb_score&page=1`;
...
async function afficherModale(p_url_film) {
  try {
    ...
    cloneTemplateDetail.querySelector(".titre-film").innerText =
      filmDetail.title;
    ...
    cloneTemplateDetail.querySelector(".annee-film").innerText =
      filmDetail.year;
    cloneTemplateDetail.querySelector(".genre-film").innerText =
      filmDetail.genres;

    // Affichage de la classification
    if (filmDetail.rated !== "Not rated or unkown rating") {
      if (!isNaN(filmDetail.rated)) {
        cloneTemplateDetail.querySelector(".classification").innerText =
          "PG-" + filmDetail.rated + "  -";
      } else {
        cloneTemplateDetail.querySelector(".classification").innerText =
          filmDetail.rated + "  -";
    ...
    cloneTemplateDetail.querySelector(".duree-film").innerText =
      filmDetail.duration + " minutes   ";
    cloneTemplateDetail.querySelector(".pays").innerText =
      "  (" + filmDetail.countries + ")";
    cloneTemplateDetail.querySelector(".score-film").innerText =
      "IMDB score : " + filmDetail.imdb_score + "/10";

    // Préparation des données monnaitaires pour affichage
    if (
      filmDetail.worldwide_gross_income !== null &&
      filmDetail.budget_currency !== null
    ) {
      ...
      cloneTemplateDetail.querySelector(".recettes").innerText =
        "Recettes au box-office : " + symbol + recettesFormatees + "m";
    } else {
      cloneTemplateDetail.querySelector(".recettes").innerText =
        "Recettes au box-office non communiquées";
    }

    cloneTemplateDetail.querySelector(".realisateur-film").innerText =
      filmDetail.directors;
    cloneTemplateDetail.querySelector(".description-longue-film").innerText =
      filmDetail.long_description;
    cloneTemplateDetail.querySelector(".acteurs-film").innerText =
      filmDetail.actors;
...
function ajouterBoutonVoirPlus(p_cloneTemplate) {
  ...
  bouton.addEventListener("click", () => {
    if (bouton.innerText === "Voir plus") {
      ...
      const articlesAffiches = p_cloneTemplate.querySelectorAll(
        "article.est_affiche",
      );
